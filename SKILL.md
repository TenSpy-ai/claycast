...
| Tables | `create_table`, `list_tables` (regular tables + function tables, each tagged `_kind`), `list_function_tables` (function/subroutine tables only — they are ABSENT from `GET /workspaces/{ws}/tables`), `list_folders`, `get_table`, `get_schema`, `get_field_map`, `count_records`, `inspect_table`, `delete_table`, `set_table_description`, `generate_table_description` (built-in AI) |
...
- **ANY non-concatenation expression in an `http-api-v2` `body` makes the column's inputs INVISIBLE in the Clay UI.** The column runs fine and sends the right payload — but opening it in Clay shows a blank body, so the config can't be read or edited. Verified 2026-08-19 by a 7-column probe sweep: `||` (inside OR outside `Clay.formatForJSON`), `??`, ternaries, `==` and arithmetic ALL render blank; only a body of pure string literals + `+` + `{{field}}` / `Clay.formatForJSON({{field}})` renders. Clay's API accepts them all with no `settingsError` — the failure is client-side only. Move every expression into its own formula column and reference it. **Exemption: path accessors on a field ref (`{{Field}}?.key`, `{{Field}}.key`, `{{Field}}?.[0]`) are SAFE** — they are part of the reference token, which is how Clay's UI itself writes a reference into a JSON column; only expressions that *compute on a value* break. `format_json_body()` and `create_action_column()` raise on the operators, after stripping field refs + their accessors so a nested reference is not a false positive.
...
- **Type-mismatched gate comparisons are silently DEAD — unconditional spend under AUTO_RUN (verified 2026-08-13).** Both directions bite: (a) a TEXT formula gate feeder stringifies boolean results — the stored `'false'` is truthy, so `{{gate}}` / `!{{gate}}` never block; the feeder must emit `"true"`/`""` via ternary. (b) A BOOLEAN-typed column compared to a string literal (`{{bool}} != "true"`) is ALWAYS true (numeric coercion; `1 != NaN`) — use `!{{bool}}`. Audit rule: resolve each referenced field's `dataTypeSettings.type` before trusting any gate comparison. Details: clay-api-reference.md § Conditional Execution → "Type-mismatch dead gates".

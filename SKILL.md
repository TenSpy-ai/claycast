...
- **A `||` inside an `http-api-v2` `body` makes the column's inputs INVISIBLE in the Clay UI.** The column runs fine and sends the right payload — but opening it in Clay shows a blank body, so the config can't be read or edited. Verified 2026-08-19 by controlled A/B (two columns identical but for `( {{f}} || "NONE" )`; the `||` one renders blank). Keep bodies a pure concatenation and move any fallback into its own formula column. `format_json_body()` and `create_action_column()` now raise on it.

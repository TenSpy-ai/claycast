...
- **inputs:**
  ...
  - `body`: schema type is **longtext**. UI-canonical form = `formulaText` building a JSON string with
    ...
    **Keep the body a PURE concatenation — no `||`, `??`, ternaries, `==`, arithmetic or any other
    expression inside it**, or the Clay UI renders the column's inputs blank (all five operator classes
    verified blank 2026-08-19). Put any fallback or computation in its own formula column and reference
    that column instead.
...
- **gotchas:**
  ...
  - **NO value-computing expressions inside a `body` formulaText — they make the column's inputs UNREADABLE in the Clay UI.** Verified 2026-08-19 by a 7-column probe sweep: `||` (inside or outside `Clay.formatForJSON`), `??`, ternaries, `==`, and arithmetic all render blank; only string literals + `+` + `{{field}}` / `Clay.formatForJSON({{field}})` renders. The column still runs correctly and returns the right payload, and Clay's API reports no `settingsError` — the failure is client-side only, so nobody can see or edit the request. **Path accessors on a field ref are exempt and safe** — `{{Field}}?.key` / `.key` / `?.[0]` are part of the reference token (this is how Clay's UI addresses a JSON column) and render fine. Move every real expression into a formula column. See clay-api-reference.md § "`||` in an http-api-v2 body renders the column unreadable in the UI".

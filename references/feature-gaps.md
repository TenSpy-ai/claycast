...
- `CLOSED 2026-04-30:` Find People AND Find Companies sourced-table creation is now available via `clay.preview_sourced_table(...)` + `clay.create_sourced_table(...)` (preview half REGRESSED 2026-07-23 — see below). Both `cpj_type="people"` and `cpj_type="companies"` are live-verified (workspace 12345).
- `REGRESSED 2026-07-23:` the PREVIEW half of the sourced-table line above is dead — Clay removed the `*-preview` enrichmentTypes from the `run-enrichment` server allowlist, so `clay.preview_sourced_table(...)` now 400s. `create_sourced_table(...)` still works. Free-preview replacement: the official `clay` CLI search (`clay search filters-mode create/run`) or the public API `/public/v0/search/filters-mode` — both free.
...
**Status as of 2026-05-01:** `CLOSED` for both People AND Companies variants. ClayCast wraps the flow via `clay.preview_sourced_table(...)` and `clay.create_sourced_table(...)` for either entity type. **Update 2026-07-23: the preview half REGRESSED** — the `*-preview` enrichmentTypes were removed from the run-enrichment allowlist and `preview_sourced_table` now 400s; `create_sourced_table(...)` remains live (see "Preview-without-saving" below). The full request/response contract — `cpjConfig`, `basicFields`, the captured inputs schema for both entity types — is documented in `clay-api-reference.md` → "Find People / Find Companies sourced-table creation".
...
**Preview-without-saving — REGRESSED 2026-07-23 (capability lost, history kept):** `POST /v3/actions/run-enrichment` with `enrichmentType: "<actionKey>-preview"` USED to return `{result: {people|companies: [...rows...], peopleCount|companiesCount: N}, metadata: {additionalCreditCost: 0}, ...}` for ANY filter combination at 0 credits (verified live 2026-05-01; hard-capped at exactly `50`, ClayCast raised before network on `limit > 50`). As of 2026-07-23 the server allowlist dropped all `*-preview` types and the call 400s — `clay.preview_sourced_table(...)` is DEAD. The zero-credit-preview capability now lives on the OFFICIAL surface only: `clay search filters-mode create/run` (CLI) or the public API `/public/v0/search/filters-mode`, both free. Details: clay-api-reference.md → run-enrichment allowlist section.

...
- **Auto-fit column widths (added 2026-08-06):** `autofit_view_fields(table_id, view_id, sample_rows=50, min_width=80, max_width=480)` sizes every VISIBLE column so header + sampled cell text are readable: width = clamp(pad + ~7.2px/char × the longer of column name vs longest cell LINE in the sample); multi-line cells measure their longest line, dict/list cells are JSON-stringified, long-text columns pin at max_width. One `set_view_fields` PATCH, idempotent. First live run: 85 columns in one call, all widths verified persisted.
